...
| `INFLUENZA_TYPE_YW.csv` | 實驗室自動通報系統（LARS） A 型 / B 型 / 未分型陽性數 | 週 × 縣市 × 鄉鎮 × 68 家醫院 | 201453–202603 | 型別領先指標 |
...
| 為何從 2016 起 | 健保急診（201552 起）、LARS 實驗室自動通報（201453 起）、RODS、NIDDS、健保門診/住院在此區間**全部完整、無缺週**，可建立單一對齊的多來源面板 |
...
2026-09-17 第二次更新：十個原始檔皆更新至 2026w37（9/6–9/12 為最後完整週），假日檔延伸至 2026-12-27。自動偵測結果：健保門診/急診、RODS、實驗室完整至 **202636**，NIDDS 完整至 202633（發病週回補保守多刪 3 週）。下表為 2 月版資料的覆蓋，保留作紀錄。
...
| LARS 實驗室自動通報 | 201453–202603 | 201502–202602 | 201453、201501、202603 |
...
- **RODS 急診類流感就診百分比**（`rods_ili_pct_REPORT.md`）：COVID 後最佳為 tfm_expanding_sym，WIS 0.63 個百分點，對 naive 0.72 改善 12%，AutoETS 0.77，季節性 naive 2.0；MAPE 6.8%，MASE 0.25，80% 涵蓋率 0.80。以流行閾值 11% 計：閾值命中率 0.82、誤報率 0.19（naive 0.78 / 0.24）。
...
**2026-09-17 更新（資料至 202636）**：2026 年夏末出現流感波，202636 全國類流感門診 122,247 人次（就診率 2.15%），是 2016 年以來第 27–40 週的第 3 高，RODS 急診類流感% 連續 6 週 ≥ 10%（202636 為 12.0%），實驗室 A 型陽性數 2,421、陽性率 8.1%，A 型占 98%。聯合預測（起點 202636）：門診 202637–202640 中位數 129,534 → 140,864 → 139,316 → 132,976（第 1 週 80% 區間 115,472–144,328）；RODS% 12.4 → 12.9 → 12.7 → 12.4；急診人次 11,896 → 13,052 → 13,879 → 12,729；重症（起點 202633）65 → 61 → 57 → 54。第 2–4 週各含 1 天平日假日（中秋、教師節補假、國慶）。以下為 2 月版資料的即時預測，保留作紀錄。
...
- 資料來源註記（2026-09-17）：目前的實驗室 A/B 型資料來自實驗室自動通報系統（LARS），合約實驗室的資料日後再加入。
...
- `thr_hit_rate` 閾值命中率（敏感度）：實際 ≥ 閾值的週中，中位數也 ≥ 閾值的比例；另報誤報率與整體準確率。閾值：RODS 急診類流感就診百分比用流行閾值 **11%**（使用者指定，`backtest.DEFAULT_THRESHOLDS`；2026-09-17 由 10% 修正為 11%），其他目標預設為資料窗內實際值第 75 百分位（門診 79,955 人次）；可用 `make_report.py --threshold` 覆寫。
...
其他層級（COVID 後 h = 1–4 平均）：RODS 急診% 最佳設定方向命中率 0.68（naive 0.48）、以 11% 為閾值的閾值命中率 0.82（naive 0.78）、準確率 0.82（0.77）；22 縣市聯合 + 共變數 0.64（naive 0.52）；18 年齡層聯合 + 共變數 0.68（naive 0.53）。完整表格在各 `*_REPORT.md` 的「命中率」一節。
